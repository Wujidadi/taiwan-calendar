...
import { moonSunDiffToTimeFaster, findSunRiseOrSet, newMoonOrdinal } from './ephemeris'
import {
...
export function fastSolarEclipseSearch(jd: number): FastEclipseResult {
  ...
  const t4 = t3 * t
  const L0 =
    ((93.2720993 + 483202.0175273 * t - 0.0034029 * t2 - t3 / 3526000 + t4 / 863310000) / 180) *
    Math.PI
  re.ac = 1
  re.lx = 'N'
  ...
  const r2 = mk - f2 * mR
  const b = 0.9972
  const Agm = Math.abs(gm)
  const Ar2 = Math.abs(r2)
  const fh2 = mR - mk / f2
  const h = Agm < 1 ? Math.sqrt(1 - gm * gm) : 0
...
export class SolarEclipseBesselian {
  ...
  init(jd: number, n: number): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      S = [
        S[0] + zd[0] + sunLongitudeAberration(T) + Math.PI,
        -S[1] + sunLatitudeAberration(T),
        S[2],
      ]
  ...
  private chazhi(jd: number, xt: number): readonly number[] {
    ...
    for (let i = 0; i < m; i++) {
      z[i] =
        ...
        ((B[base + i + w]! -
          B[base + i - w]! +
          (B[base + i + w]! + B[base + i - w]! - B[base + i]! * 2) * tAdj) *
          tAdj) /
          2
    }
    return z
  ...
  bseM(jd: number): readonly [number, number, number] {
    ...
    return [
      a[0] / EARTH_EQUATORIAL_RADIUS_KM,
      a[1] / EARTH_EQUATORIAL_RADIUS_KM,
      a[2] / EARTH_EQUATORIAL_RADIUS_KM,
    ]
  }
  ...
  feature(jd: number): FeatureResult {
    ...
    const F = lineEarthIntersectBessel(
      xc,
      yc,
      2,
      xc,
    ...
    const ls1 = 1
    let dt1 = 0
    if (d < ls1) dt1 = Math.sqrt(ls1 * ls1 - d * d) / v
    const t2a = t0 - dt1
    const t3a = t0 + dt1
    const ls2 = 1 + Bc.r1
    let dt2 = 0
    if (d < ls2) dt2 = Math.sqrt(ls2 * ls2 - d * d) / v
    const t4 = t0 - dt2
    const t5 = t0 + dt2
    const t6 = -b[0] / vx
    ...
    let lx: string
    if (F.W === 100) {
      const ls = this.bse2db([xc, yc, 0], I, false)
      zxJ = ls[0]
      zxW = ls[1]
  ...
  private nanbei(
    ...
    const p = lineEarthIntersectBessel(
      X,
      Y,
      M[2],
      x,
  ...
  private mQie(
    ...
    if (fPrev !== f) {
      ...
      if (g.n) {
        ...
        const dbF = this.bse2db(F as unknown as [number, number, number], I2, true)
        this.push([dbF[0], dbF[1]], A)
      }
    }
  ...
  private mDian(
    ...
    let p = {
      n: 0,
      A: undefined as unknown as [number, number],
      B: undefined as unknown as [number, number],
      R1: 0,
      R2: 0,
    }
    for (let i = 0; i < 2; i++) {
  ...
  jieX(jd: number): FeatureResult {
    ...
    for (let i = 0; i <= N; i++, t += dt) {
      ...
      if (p.n) {
        ...
        if (n1 === 1) {
          this.push([pAdb[0], pAdb[1]], re.p1)
          this.push([pBdb[0], pBdb[1]], re.p2)
        }
        if (n1 === 3) {
          this.push([pAdb[0], pAdb[1]], re.p3)
          this.push([pBdb[0], pBdb[1]], re.p4)
        }
      }

      if (!this.mDian(M, vx, vy, false, r, I, Ua)) {
        if (Ua.length > 0) Ua = re.q3
      }
      if (!this.mDian(M, vx, vy, true, r, I, Ub)) {
  ...
  jieX3(jd: number): string {
    ...
    for (let i = 0; i < N; i++, t += dt) {
      ...
      const r = B.r1
      const I = this.bse(t)
      let s2 = formatJD((t + J2000) as JulianDay) + ' '
      let k = 0
      const nb = (h: 1 | -1, r0: number): string => {
        const p = this.nanbei(M, vx, vy, h, r0, I)
        if (p[1] !== 100) {
          k++
          return formatRadianToMinute(p[0]) + ' ' + formatRadianToMinute(p[1]) + '|'
        }
        return '-------------------|'
      }
      s2 += nb(+1 as 1, r)
      ...
      if (pC[1] !== 100) {
        k++
        s2 += formatRadianToMinute(pC[0]) + ' ' + formatRadianToMinute(pC[1]) + '|'
      } else s2 += '-------------------|'
      s2 += nb(-1 as -1, B.r2)
      s2 += nb(-1 as -1, r).replace('|', ' ')
      if (k) s += s2 + '\n'
    }
...
export class SolarEclipseLocal {
  ...
  secMax(jd: number, L: number, fa: number, high: number): LocalEclipseResult {
    ...
    if (rmin <= G.mr + G.sr) {
      ...
      V1 = normalizeAngle(
        P1 - parallacticAngle(meanSiderealTimeFromTD(sT[0]!), L, fa, g0.sCJ, g0.sCW),
      )
      ...
      V2 = normalizeAngle(
        P2 - parallacticAngle(meanSiderealTimeFromTD(sT[2]!), L, fa, g2.sCJ, g2.sCW),
      )
    }
    if (rmin <= G.mr - G.sr) {
      LX = '全'
