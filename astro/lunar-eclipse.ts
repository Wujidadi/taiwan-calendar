...
export interface LunarEclipseState {
  T: number; L: number; fa: number
  dt: number; jd: number; dL: number; dE: number; E: number; gst: number
  // 月亮
  mHJ: number; mHW: number; mR: number
  mCJ: number; mCW: number
  mShiJ: number
  mCJ2: number; mCW2: number; mR2: number
  mDJ: number; mDW: number
  mPJ: number; mPW: number
  // 太陽
  sHJ: number; sHW: number; sR: number
  sCJ: number; sCW: number
  sShiJ: number
  sCJ2: number; sCW2: number; sR2: number
  sDJ: number; sDW: number
  sPJ: number; sPW: number
  // 其他
  sc: number; pty: number; zty: number
  mRad: number; sRad: number
  e_mRad: number; eShadow: number; eShadow2: number
  mIll: number
  zx_J: number; zx_W: number
}

/** 月食 lecXY 計算結果 */
interface EclipseXY {
  x: number; y: number; t: number
  mr: number; er: number; Er: number
  e_mRad: number; eShadow: number; eShadow2: number
}
...
export function calcLunarEclipse(
  ...
  z = [normalizeAngle(z[0] + moonLongitudeAberration(Tcy) + dL), z[1] + moonLatitudeAberration(Tcy), z[2]]
  const mHJ = z[0]; const mHW = z[1]; const mR = z[2]

  z = rotateSpherical(z, E)
  const mCJ = z[0]; const mCW = z[1]

  let mShiJ = normalizeAngle(gst + longitude - z[0])
  if (mShiJ > Math.PI) mShiJ -= TWO_PI

  z = applyParallax(z, mShiJ, latitude, elevationKm)
  const mCJ2 = z[0]; const mCW2 = z[1]; const mR2 = z[2]

  z = [z[0] + Math.PI / 2 - gst - longitude, z[1], z[2]]
  z = rotateSpherical(z, Math.PI / 2 - latitude)
  z = [normalizeAngle(-Math.PI / 2 - z[0]), z[1], z[2]]
  const mDJ = z[0]; const mDW = z[1]
  const mPJ = z[0]
  const mPW = z[1] > 0 ? z[1] + refractionFromTrueAltitude(z[1]) : z[1]

  // ── 太陽 ──
  let sz = earthCoord(Tcy, -1, -1, -1)
  sz = [normalizeAngle(sz[0] + Math.PI + sunLongitudeAberration(Tcy) + dL), -sz[1] + sunLatitudeAberration(Tcy), sz[2]]
  const sHJ = sz[0]; const sHW = sz[1]; const sR = sz[2]

  sz = rotateSpherical(sz, E)
  const sCJ = sz[0]; const sCW = sz[1]
  ...
  const eShadow
    = ((EARTH_MEAN_RADIUS_KM / mR) * RAD_TO_ARCSEC - (959.63 - 8.794) / sR) * 51 / 50
  const eShadow2
    = ((EARTH_MEAN_RADIUS_KM / mR) * RAD_TO_ARCSEC + (959.63 + 8.794) / sR) * 51 / 50
  ...
  return {
    T, L: longitude, fa: latitude, dt, jd, dL, dE, E, gst,
    mHJ, mHW, mR, mCJ, mCW, mShiJ, mCJ2, mCW2, mR2, mDJ, mDW, mPJ, mPW,
    sHJ, sHW, sR, sCJ, sCW, sShiJ, sCJ2, sCW2, sR2, sDJ, sDW, sPJ, sPW,
    sc, pty, zty, mRad, sRad, e_mRad, eShadow, eShadow2, mIll, zx_J, zx_W,
  }
}
...
function lecXY(jd: number): EclipseXY {
  ...
  zs = [normalizeAngle(zs[0] + Math.PI + sunLongitudeAberration(T)), -zs[1] + sunLatitudeAberration(T), zs[2]]
  let zm = moonCoord(T, -1, -1, -1)
  zm = [normalizeAngle(zm[0] + moonLongitudeAberration(T)), zm[1] + moonLatitudeAberration(T), zm[2]]
...
export function lecMax(jd: number): LunarEclipseTimings {
  ...
  jd = moonSunDiffToTimeFaster(
    Math.floor((jd - 4) / 29.5306) * Math.PI * 2 + Math.PI,
  ) * 36525

  // 粗略求極值
  const u0
    = (-18461 * Math.sin(0.057109 + 0.23089571958 * jd) * 0.2309) / RAD_TO_ARCSEC
  const v0
    = (moonAngularVelocity(jd / 36525) - earthAngularVelocity(jd / 36525)) / 36525
  let G = lecXY(jd)
