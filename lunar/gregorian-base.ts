...
export const SOLAR_DATE_FESTIVALS: string[][] = SOLAR_DATE_FESTIVALS_RAW.split('|').map(s =>
  s.split(','),
)
...
export function getDayName(u: Record<string, unknown>, r: Record<string, unknown>): void {
  ...
  for (let i = 0; i < monthFtv.length; i++) {
    ...
    if (s.substring(5, 6) === '-') {
      if (
        Number(u['year']) < Number(s.substring(1, 5)) ||
        Number(u['year']) > Number(s.substring(6, 10))
      )
        continue
      s = s.substring(10)
    } else {
    ...
    if (type === '#') {
      r['holidayA'] = String(r['holidayA']) + s + ' '
      r['isHoliday'] = 1
    }
  ...
  // 按週查找：本月的第 N 個星期 W
  let w = Number(u['weekIndex'])
  if (Number(u['weekday']) >= Number(u['firstWeekday'])) w += 1
  let w2 = w
  if (Number(u['weekIndex']) === Number(u['weekCount']) - 1) w2 = 5
