...
const FESTIVAL: Record<string, string> = {
  ...
  '火把节、星回节(彝、白、佤、阿昌、纳西、基诺族 )':
    '火把節、星回節（彝、白、佤、阿昌、納西、基諾族）',
...
export function getLunarDayName(u: Record<string, unknown>, r: Record<string, unknown>): void {
  ...
  if (u.lunarLeap !== LEAP_PREFIX) {
    if (d === '正月初一') {
      r.holidayA = String(r.holidayA ?? '') + festival('春节') + ' '
      r.isHoliday = 1
    }
    if (d === '正月初二') {
      r.holidayB = String(r.holidayB ?? '') + festival('大年初二') + ' '
      r.isHoliday = 1
    }
    if (d === '五月初五') {
      r.holidayA = String(r.holidayA ?? '') + festival('端午节') + ' '
      r.isHoliday = 1
    }
    if (d === '八月十五') {
      r.holidayA = String(r.holidayA ?? '') + festival('中秋节') + ' '
      r.isHoliday = 1
    }
    if (d === '正月十五') {
    ...
    if (d === '正月十六')
      r.holidayC = String(r.holidayC ?? '') + festival('侗族芦笙节(至正月二十)') + ' '
    ...
    if (d === '三月十五')
      r.holidayC = String(r.holidayC ?? '') + festival('白族三月街(至三月二十)') + ' '
    ...
    if (d === '六月廿四')
      r.holidayC =
        String(r.holidayC ?? '') + festival('火把节、星回节(彝、白、佤、阿昌、纳西、基诺族 )') + ' '
    if (d === '七月初七')
      r.holidayB = String(r.holidayB ?? '') + festival('七夕(中国情人节,乞巧节,女儿节 )') + ' '
  ...
  if (u.lunarNextMonthName === '正') {
    if (d === '十二三十' && u.lunarMonthLength === 30) {
      r.holidayA = String(r.holidayA ?? '') + festival('除夕') + ' '
      r.isHoliday = 1
    }
    if (d === '十二廿九' && u.lunarMonthLength === 29) {
      r.holidayA = String(r.holidayA ?? '') + festival('除夕') + ' '
      r.isHoliday = 1
    }
  ...
  if (u.solarTermLabel) {
    if (u.solarTermLabel === '清明') {
      r.holidayA = String(r.holidayA ?? '') + String(u.solarTermLabel) + ' '
      r.isHoliday = 1
  ...
  if (daysSinceDongzhi >= 0 && daysSinceDongzhi < 81) {
    // 數九
  ...
  if (daysSinceXiazhi >= 20 && daysSinceXiazhi < 30 && stem === '庚')
    r.holidayB = String(r.holidayB ?? '') + festival('初伏') + ' '
  if (daysSinceXiazhi >= 30 && daysSinceXiazhi < 40 && stem === '庚')
    r.holidayB = String(r.holidayB ?? '') + festival('中伏') + ' '
  if (daysSinceLiqiu >= 0 && daysSinceLiqiu < 10 && stem === '庚')
    r.holidayB = String(r.holidayB ?? '') + festival('末伏') + ' '
  if (daysSinceMangzhong >= 0 && daysSinceMangzhong < 10 && stem === '丙')
    r.holidayB = String(r.holidayB ?? '') + festival('入梅') + ' '
  if (daysSinceXiaoshu >= 0 && daysSinceXiaoshu < 12 && branch === '未')
    r.holidayB = String(r.holidayB ?? '') + festival('出梅') + ' '
}
...
export function computeBazi(jd: number, J: number, ob: Record<string, unknown>): void {
  ...
  ob.baziYear = (HEAVENLY_STEMS[v % 10] ?? '') + (EARTHLY_BRANCHES[v % 12] ?? '')
  v = k + 2 + 60000000
  ob.baziMonth = (HEAVENLY_STEMS[v % 10] ?? '') + (EARTHLY_BRANCHES[v % 12] ?? '')
  v = D - 6 + 9000000
  ob.baziDay = (HEAVENLY_STEMS[v % 10] ?? '') + (EARTHLY_BRANCHES[v % 12] ?? '')
  v = (D - 1) * 12 + 90000000 + SC
  ob.baziHour = (HEAVENLY_STEMS[v % 10] ?? '') + (EARTHLY_BRANCHES[v % 12] ?? '')
