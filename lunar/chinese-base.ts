...
const LUNAR_NUMERALS   = ['零','一','二','三','四','五','六','七','八','九','十']
const LEAP_PREFIX      = '閏'

// ── 年號表（紀年資料）──
//
// 結構：每 7 個元素描述一個年號，依序為
//   [起始公元(int), 使用年數(int), 已用年數(int), 朝代, 朝號, 皇帝, 年號]
//
const REIGN_TITLES_RAW =
...
+'1862,13,0,清,穆宗,爱新觉罗载淳,同治,1875,34,0,清,德宗,爱新觉罗载湉,光绪,1909,3,0,清,无朝,爱新觉罗溥仪,宣统,1912,37,0,近、現代,中華民國,,民國,1949,9999,1948,當代,中國,,公元'

// 針對年號字串套用繁體中文轉換（僅對近現代及當代條目適用，其餘歷史字符已為傳統形式）
...
function parseReignTitles(raw: string): (string | number)[] {
  const arr: (string | number)[] = raw.split(',')
  for (let i = 0; i < arr.length; i += 7) {
    arr[i]     = Number(arr[i])
    arr[i + 1] = Number(arr[i + 1])
    arr[i + 2] = Number(arr[i + 2])
    // 對字串欄位套用繁體轉換
    arr[i + 3] = toTraditional(arr[i + 3] as string)
    arr[i + 4] = toTraditional(arr[i + 4] as string)
    arr[i + 5] = toTraditional(arr[i + 5] as string)
    arr[i + 6] = toTraditional(arr[i + 6] as string)
  }
  return arr
}

export const REIGN_TITLES = parseReignTitles(REIGN_TITLES_RAW)

// 取得年號（依公元年），可能跨多朝代，回傳串接字串（以 `;` 分隔）。
export function getReignTitle(y: number): string {
  let s = ''
  const ob = REIGN_TITLES
  for (let i = 0; i < ob.length; i += 7) {
    const j = ob[i] as number
    if (y < j || y >= j + (ob[i + 1] as number)) continue
    const c = String(ob[i + 6]) + (y - j + 1 + (ob[i + 2] as number)) + '年' // 年號及年次
    s += (s ? ';' : '') + '[' + String(ob[i + 3]) + ']' + String(ob[i + 4]) + ' ' + String(ob[i + 5]) + ' ' + c
  }
  return s
}
...
function festival(key: string): string {
  return FESTIVAL[key] ?? key
}

// 計算農曆節日。
//   u 為日物件（lunarMonthName / lunarDayName / lunarLeap / lunarNextMonthName / lunarMonthLength /
//     solarTermLabel / lunarDayGanZhi / daysSinceDongzhi 等欄位）。
//   r 為輸出物件，追加 holidayA / holidayB / holidayC 與 isHoliday。
export function getLunarDayName(u: Record<string, unknown>, r: Record<string, unknown>): void {
  // 按農曆日期查找重要節假日
  const lunarMonthName = u.lunarMonthName as string
  const lunarDayName   = u.lunarDayName   as string
  const d = lunarMonthName + (lunarMonthName.length < 2 ? '月' : '') + lunarDayName
  if (u.lunarLeap !== LEAP_PREFIX) {
    if (d === '正月初一') { r.holidayA = String(r.holidayA ?? '') + festival('春节') + ' ';     r.isHoliday = 1 }
    if (d === '正月初二') { r.holidayB = String(r.holidayB ?? '') + festival('大年初二') + ' '; r.isHoliday = 1 }
    if (d === '五月初五') { r.holidayA = String(r.holidayA ?? '') + festival('端午节') + ' ';   r.isHoliday = 1 }
    if (d === '八月十五') { r.holidayA = String(r.holidayA ?? '') + festival('中秋节') + ' ';   r.isHoliday = 1 }
    if (d === '正月十五') {
      r.holidayA = String(r.holidayA ?? '') + festival('元宵节') + ' '
      r.holidayB = String(r.holidayB ?? '') + festival('上元节') + ' '
      r.holidayC = String(r.holidayC ?? '') + festival('壮族歌墟节 苗族踩山节 达斡尔族卡钦') + ' '
    }
    if (d === '正月十六') r.holidayC = String(r.holidayC ?? '') + festival('侗族芦笙节(至正月二十)') + ' '
    if (d === '正月廿五') r.holidayC = String(r.holidayC ?? '') + festival('填仓节') + ' '
    if (d === '正月廿九') r.holidayC = String(r.holidayC ?? '') + festival('送穷日') + ' '
    if (d === '二月初一') r.holidayC = String(r.holidayC ?? '') + festival('瑶族忌鸟节') + ' '
    if (d === '二月初二') {
      r.holidayB = String(r.holidayB ?? '') + festival('春龙节(龙抬头)') + ' '
      r.holidayC = String(r.holidayC ?? '') + festival('畲族会亲节') + ' '
    }
    if (d === '二月初八') r.holidayC = String(r.holidayC ?? '') + festival('傈傈族刀杆节') + ' '
    if (d === '三月初三') {
      r.holidayB = String(r.holidayB ?? '') + festival('北帝诞') + ' '
      r.holidayC = String(r.holidayC ?? '') + festival('苗族黎族歌墟节') + ' '
    }
    if (d === '三月十五') r.holidayC = String(r.holidayC ?? '') + festival('白族三月街(至三月二十)') + ' '
    if (d === '三月廿三') r.holidayB = String(r.holidayB ?? '') + festival('天后诞 妈祖诞') + ' '
    if (d === '四月初八') r.holidayB = String(r.holidayB ?? '') + festival('牛王诞') + ' '
    if (d === '四月十八') r.holidayC = String(r.holidayC ?? '') + festival('锡伯族西迁节') + ' '
    if (d === '五月十三') {
      r.holidayB = String(r.holidayB ?? '') + festival('关帝诞') + ' '
      r.holidayC = String(r.holidayC ?? '') + festival('阿昌族泼水节') + ' '
    }
    if (d === '五月廿二') r.holidayC = String(r.holidayC ?? '') + festival('鄂温克族米阔鲁节') + ' '
    if (d === '五月廿九') r.holidayC = String(r.holidayC ?? '') + festival('瑶族达努节') + ' '
    if (d === '六月初六') {
      r.holidayB = String(r.holidayB ?? '') + festival('姑姑节 天贶节') + ' '
      r.holidayC = String(r.holidayC ?? '') + festival('壮族祭田节 瑶族尝新节') + ' '
    }
    if (d === '六月廿四') r.holidayC = String(r.holidayC ?? '') + festival('火把节、星回节(彝、白、佤、阿昌、纳西、基诺族 )') + ' '
    if (d === '七月初七') r.holidayB = String(r.holidayB ?? '') + festival('七夕(中国情人节,乞巧节,女儿节 )') + ' '
    if (d === '七月十三') r.holidayC = String(r.holidayC ?? '') + festival('侗族吃新节') + ' '
    if (d === '七月十五') r.holidayB = String(r.holidayB ?? '') + festival('中元节 鬼节')
    if (d === '九月初九') r.holidayB = String(r.holidayB ?? '') + festival('重阳节') + ' '
    if (d === '十月初一') r.holidayB = String(r.holidayB ?? '') + festival('祭祖节(十月朝)') + ' '
    if (d === '十月十五') r.holidayB = String(r.holidayB ?? '') + festival('下元节') + ' '
    if (d === '十月十六') r.holidayC = String(r.holidayC ?? '') + festival('瑶族盘王节') + ' '
    if (d === '十二初八') r.holidayB = String(r.holidayB ?? '') + festival('腊八节') + ' '
  }
  if (u.lunarNextMonthName === '正') {
    if (d === '十二三十' && u.lunarMonthLength === 30) { r.holidayA = String(r.holidayA ?? '') + festival('除夕') + ' '; r.isHoliday = 1 }
    if (d === '十二廿九' && u.lunarMonthLength === 29) { r.holidayA = String(r.holidayA ?? '') + festival('除夕') + ' '; r.isHoliday = 1 }
    if (d === '十二廿三') r.holidayB = String(r.holidayB ?? '') + festival('小年') + ' '
  }
  if (u.solarTermLabel) {
    if (u.solarTermLabel === '清明') { r.holidayA = String(r.holidayA ?? '') + String(u.solarTermLabel) + ' '; r.isHoliday = 1 }
    else                              r.holidayB = String(r.holidayB ?? '') + String(u.solarTermLabel) + ' '
  }

  // 農曆雜節
  let w: string
  const daysSinceDongzhi = u.daysSinceDongzhi as number
  if (daysSinceDongzhi >= 0 && daysSinceDongzhi < 81) { // 數九
    w = LUNAR_NUMERALS[Math.floor(daysSinceDongzhi / 9) + 1] ?? ''
    if (daysSinceDongzhi % 9 === 0) r.holidayB = String(r.holidayB ?? '') + '『' + w + '九』 '
    else                             r.holidayC = String(r.holidayC ?? '') + w + '九第' + (daysSinceDongzhi % 9 + 1) + '天 '
  }

  const lunarDayGanZhi = u.lunarDayGanZhi as string
  const stem   = lunarDayGanZhi.substring(0, 1)
  const branch = lunarDayGanZhi.substring(1, 2)
  const daysSinceXiazhi   = u.daysSinceXiazhi   as number
  const daysSinceLiqiu    = u.daysSinceLiqiu     as number
  const daysSinceMangzhong = u.daysSinceMangzhong as number
  const daysSinceXiaoshu  = u.daysSinceXiaoshu   as number
  if (daysSinceXiazhi   >= 20 && daysSinceXiazhi   < 30 && stem   === '庚') r.holidayB = String(r.holidayB ?? '') + festival('初伏') + ' '
  if (daysSinceXiazhi   >= 30 && daysSinceXiazhi   < 40 && stem   === '庚') r.holidayB = String(r.holidayB ?? '') + festival('中伏') + ' '
  if (daysSinceLiqiu    >= 0  && daysSinceLiqiu    < 10 && stem   === '庚') r.holidayB = String(r.holidayB ?? '') + festival('末伏') + ' '
  if (daysSinceMangzhong >= 0 && daysSinceMangzhong < 10 && stem   === '丙') r.holidayB = String(r.holidayB ?? '') + festival('入梅') + ' '
  if (daysSinceXiaoshu  >= 0  && daysSinceXiaoshu  < 12 && branch === '未') r.holidayB = String(r.holidayB ?? '') + festival('出梅') + ' '
}

// 命理八字。jd 為格林尼治 UT（J2000 起算），J 為本地經度（弧度）。
// 結果寫入 ob.baziTrueSolarTime / baziYear / baziMonth / baziDay / baziHour / baziHoursAll。
export function computeBazi(jd: number, J: number, ob: Record<string, unknown>): void {
  const jd2 = jd + deltaT(jd) // 力學時
  const w = sunApparentLongitude(jd2 / 36525, -1) // 此刻太陽視黃經
  const k = Math.floor((w / TWO_PI * 360 + 45 + 15 * 360) / 30) // 1984 年立春起算的節氣數（不含中氣）
  jd += equationOfTimeFast(jd2 / 36525) + J / Math.PI / 2 // 本地真太陽時
  ob.baziTrueSolarTime = formatTimeOfDay(jd as Parameters<typeof formatTimeOfDay>[0])

  jd += 13 / 24 // 轉為前一日 23 點起算
  ...
  for (let i = 0; i < 13; i++) { // 一天中包含 13 個紀時
    ...
    ob.baziHoursAll = String(ob.baziHoursAll) + (i ? ' ' : '') + c
  }
}

// 精氣（給定太陽視黃經 W，回傳節氣 JD，已加上北京時 +8h）。
export function preciseSolarTermFromLongitude(W: number): number {
  const t = sunApparentLongToTime(W) * 36525
  return t - deltaT(t) + 8 / 24
}

// 精朔（給定月日視黃經差 W，回傳合朔 JD，已加上北京時 +8h）。
export function preciseNewMoonFromLongitude(W: number): number {
  const t = moonSunDiffToTime(W) * 36525
  return t - deltaT(t) + 8 / 24
}

// 精氣（給定 JD 求最近節氣）。
export function preciseSolarTermFromJD(jd: number): number {
  const d = Math.PI / 12
  const w = Math.floor((jd + 293) / 365.2422 * 24) * d
  const a = preciseSolarTermFromLongitude(w)
  if (a - jd > 5)  return preciseSolarTermFromLongitude(w - d)
  if (a - jd < -5) return preciseSolarTermFromLongitude(w + d)
  return a
}

// 精朔（給定 JD 求最近合朔）。
export function preciseNewMoonFromJD(jd: number): number {
  return preciseNewMoonFromLongitude(Math.floor((jd + 8) / 29.5306) * Math.PI * 2)
}
