...
import { sunApparentLongitude, moonSunApparentLongDiff } from '../astro/ephemeris'
import {
  getReignTitle,
  getLunarDayName,
...
export class LunarMonth {
  ...
  calcMonth(year: number, month: number): void {
    ...
    for (let i = 0; i < Bdn; i++) {
      ...
      if (
        !shuoQiCalculator.centralQiList.length ||
      ...
      if (jdNoon === shuoQiCalculator.newMoonList[mk]! || jdNoon === Bd0) {
        ...
        ob['lunarLeap'] =
          shuoQiCalculator.leapMonth && shuoQiCalculator.leapMonth === mk ? leapPrefix : ''
      ...
      // 干支紀年（以立春為界）
      let D =
        shuoQiCalculator.centralQiList[3]! +
        (jdNoon < shuoQiCalculator.centralQiList[3]! ? -365 : 0) +
        365.25 * 16 -
        35
      ...
      for (let j = 0; j < 14; j++) {
        if (shuoQiCalculator.monthNames[j] !== '正' || (shuoQiCalculator.leapMonth === j && j))
          continue
      ...
      D = (ob['lunarYearNum'] as number) + 12000
      ob['lunarYearGanZhi'] = stems[D % 10]! + branches[D % 12]!
      D = (ob['lunarYearNumNewYear'] as number) + 12000
      ob['lunarYearGanZhi2'] = stems[D % 10]! + branches[D % 12]!
